...
from Color import *
import Scene_3D_singleton as Scene_3D
...
sc.AddNewLight(Light(Vec_3D([10,4,11]),Vec_3D([1,1,1])))
sc.AddNewLight(Light(Vec_3D([20,100,5]),Vec_3D([1,1,1])))
sc.AddNewLight(Light(Vec_3D([20,-100,5]),Vec_3D([1,1,1])))
#设置背景 颜色
...
#%% 测试图片
sc._img_size=[100,100]
sc._plane_size=[10,10]
sphere1._c=Vec_3D([1,-5,4])
sc._UpdateRGB()
plt.close('all')
plt.figure()
plt.imshow(np.flip(sc.RayTrackImg(),0))#左上角为坐标原点,而追踪是以左下角为原点
#%%
plt.axis('off')
plt.show()
plt.savefig(str(time.time())+'img.jpg', bbox_inches='tight')
'''
#%% 测试图片
#sc.SetEye(Vec_3D([1,1,1]),Vec_3D([0,,]).Norm(),Vec_3D([0,1,0]).Norm(),3)
sc._img_size=[1000,1000]
sc._plane_size=[2,2]
sc._UpdateRGB()
img_list=[]
frames=200
